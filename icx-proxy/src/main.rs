...
async fn handle_request(
    ...
    } else if request_uri_path.starts_with("/_/") && candid_url.is_some() {
        slog::debug!(
            logger,
            "URI Request to path '{}' being forwarded to internal candid endpoint",
            &request.uri().path()
        );
        forward_api(&ip_addr, request, &candid_url.unwrap()).await
    } else {
        let agent = Arc::new(
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let candid_url = opts.candid.clone();
    ...
    let service = make_service_fn(|socket: &hyper::server::conn::AddrStream| {
        ...
        let candid_url = candid_url.clone();
